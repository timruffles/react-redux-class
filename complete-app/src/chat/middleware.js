  ...
  return (next) => {
    return handler(next, {
      [CHAT_HISTORY_FETCH]: (action) => {
        ...
        fetchChats()
          .then((result) => (
            ChatActions.historyFetched(here(toMap(result)))
          ), (err) => (
            ChatActions.historyFetched(failed(err))
          ))
          .then(dispatch)
      ...
      [SEND_MESSAGE]: (action) => {
        ...
        const sent = sendChat(chat);
        next(ChatActions.message(chat));

        sent
          .then((result) => (
            ChatActions.sendResult(chat.id, here({ok: true}))
          ), (err) => (
            ChatActions.sendResult(chat.id, failed(err))
          ))
          .then(dispatch)
      },
